...
StyledWindow {
    ...
    WlrLayershell.keyboardFocus: visibilities.launcher || visibilities.session || panels.dashboard.needsKeyboard || visibilities.overview ? WlrKeyboardFocus.OnDemand : WlrKeyboardFocus.None
    ...
    HyprlandFocusGrab {
        ...
        active: (visibilities.launcher && root.contentItem.Config.launcher.enabled) || (visibilities.session && root.contentItem.Config.session.enabled) || (visibilities.sidebar && root.contentItem.Config.sidebar.enabled) || (!root.contentItem.Config.dashboard.showOnHover && visibilities.dashboard && root.contentItem.Config.dashboard.enabled) || (panels.popouts.currentName.startsWith("traymenu") && (panels.popouts.current as StackView)?.depth > 1) || visibilities.overview
        windows: [root]
        onCleared: {
            visibilities.launcher = false;
            visibilities.session = false;
