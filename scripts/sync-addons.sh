...
# 清理已退役外挂，避免旧部署残留继续注册窗口、热区和 IPC。
for mod in "${RETIRED_HOOKS[@]}"; do
  rm -rf -- "$TARGET/$mod"
done
...
python3 - "$TARGET/shell.qml" "${INSTANCE_HOOKS[*]}" "${RETIRED_HOOKS[*]}" <<'PY'
import re
import sys
...
for mod in retired:
    import_pattern = re.compile(rf'^\s*import "{re.escape(mod)}"\s*$')
    type_name = "AiChat" if mod == "aichat" else mod[0].upper() + mod[1:]
    instance_pattern = re.compile(rf'^\s*{re.escape(type_name)}\s*\{{\}}\s*$')
    kept = [line for line in lines if not import_pattern.match(line) and not instance_pattern.match(line)]
